...
import { TrendingUpIcon, SquareMousePointerIcon, GlobeIcon, ZapIcon, ShieldCheckIcon } from "lucide-react";
import { ScorpioGlobe } from "@/components/scorpio-globe";

const features = [
	{
		id: "reports",
		children: <ReportsVisual />,
		className: "sm:col-span-2 md:col-span-6 p-0",
	},
	{
		id: "dashboard",
		children: <DashboardVisual />,
		className: "sm:col-span-2 md:col-span-4 p-0",
	},
	{
		id: "presence",
		children: <PresenceVisual />,
		className: "sm:col-span-2 md:col-span-2 p-0 flex flex-col items-center justify-center",
	},
	{
		id: "setup",
...
export function FeatureSection() {
	return (
		<div className="relative mx-auto grid w-full grid-cols-1 gap-4 lg:gap-6 sm:grid-cols-2 lg:grid-cols-6">
			{features.map((feature, idx) => (
				<FeatureCard className={feature.className} key={feature.id} delay={idx * 0.1}>
					{feature.children}
				</FeatureCard>
			))}
		</div>
...
function FeatureCard({
	...
	return (
		<motion.div
			...
			className={cn(
				"group relative overflow-hidden rounded-3xl border border-border/50 bg-white/70 dark:bg-card/50 backdrop-blur-xl px-8 pt-8 pb-6 hover:border-primary/40 hover:shadow-2xl hover:shadow-primary/10 hover:-translate-y-0.5 transition-all duration-500",
				className
			)}
		>
			<div className="pointer-events-none absolute inset-0 bg-[radial-gradient(rgba(0,0,0,0.15)_1px,transparent_1px)] dark:bg-[radial-gradient(rgba(255,255,255,0.25)_1px,transparent_1px)] [background-size:24px_24px] opacity-100" />

			{/* Light Mode Spotlight */}
			<motion.div
				className="pointer-events-none absolute -inset-px rounded-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100 z-0 dark:hidden"
				style={{
			...
			{/* Dark Mode Spotlight */}
			<motion.div
				className="pointer-events-none absolute -inset-px rounded-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100 z-0 hidden dark:block"
				style={{
					background: useMotionTemplate`
						radial-gradient(
							400px circle at ${mouseX}px ${mouseY}px,
...
function SetupVisual() {
	return (
		<>
			<div className="relative mx-auto flex size-32 items-center justify-center rounded-full border border-primary/20 bg-background shadow-xs outline outline-border/10 outline-offset-[6px]">
				<div className="absolute inset-0 z-10 scale-[1.5] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.1),transparent)] blur-xl" />

				{/* Rotating timer rings */}
				<motion.div
					animate={{ rotate: 360 }}
				...
				>
					<CustomTimerIcon className="size-14 fill-primary/80 relative z-20" />
				</motion.div>
			</div>

			<div className="relative mt-8 space-y-1.5 text-center">
				<FeatureTitle>2-Minute Deployment</FeatureTitle>
				<FeatureDescription>
					Create your first class in under 2 minutes after signing up into Scorpio.
				</FeatureDescription>
			</div>
		</>
	);
...
function UserBasedSecurity() {
	return (
		<>
			<div className="relative mx-auto flex size-32 items-center justify-center rounded-full border border-primary/20 bg-background shadow-xs outline outline-border/20 outline-offset-[6px] overflow-hidden">

				<div className="absolute inset-0 scale-[1.5] bg-[radial-gradient(ellipse_at_center,rgba(16,185,129,0.15),transparent)] blur-xl" />

				{/* Sweeping fingerprint scanner line */}
				<motion.div
					initial={{ top: "-20%" }}
					animate={{ top: "120%" }}
					transition={{ duration: 2.5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", repeatType: "reverse" }}
					className="absolute left-0 right-0 h-[2px] bg-emerald-400 shadow-[0_0_15px_4px_rgba(16,185,129,0.6)] z-20"
				/>

				<motion.div
					animate={{ rotate: [0, 3, -3, 0], scale: [1.15, 1.18, 1.15] }}
					transition={{ duration: 6, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
				>
					<CustomLockIcon className="size-24 relative z-10" />
				</motion.div>

			</div>

			<div className="relative mt-8 space-y-1.5 text-center">
				<FeatureTitle>Institutional Security</FeatureTitle>
				<FeatureDescription>
					FERPA-compliant infrastructure guaranteeing the privacy and security of district data.
				</FeatureDescription>
			</div>
		</>
	);
...
function ReportsVisual() {
	return (
		<div className="grid h-full w-full lg:grid-cols-[1fr_1.5fr] gap-4 overflow-hidden">
			<div className="relative z-10 space-y-6 py-10 pl-10 pr-4 flex flex-col justify-center">
				<motion.div
					animate={{ y: [0, -5, 0] }}
				...
				>
					<TrendingUpIcon className="size-5 text-primary/80" />
				</motion.div>
				<div className="space-y-2">
					<FeatureTitle className="text-xl">
						Pedagogical Analytics
					</FeatureTitle>
					<FeatureDescription className="text-base max-w-sm">
						Mission Control serves as your command center. Pinpoint individual student struggles, track class-wide completion rates, and verify Socratic mastery instantly.
					</FeatureDescription>
				</div>
			</div>

			{/* Screenshot */}
			<div className="relative flex items-center justify-end w-full h-full pt-4 md:pt-10">
				<motion.div
					animate={{ y: ["2%", "-1%", "2%"] }}
					transition={{ duration: 7, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
					className="relative w-full h-full min-h-[250px] md:min-h-[340px] ml-0 md:ml-4 rounded-tl-xl border-t border-l border-border/50 shadow-2xl bg-card/50 overflow-hidden translate-x-[2%]"
				>
					<img
						alt="Mission Control UI"
...
function DashboardVisual() {
	return (
		<div className="grid h-full sm:grid-cols-2 overflow-hidden">
			<div className="relative z-10 space-y-6 py-8 ps-8 pe-2 flex flex-col justify-center">
				<motion.div
			...
			{/* Dashboard Screen */}
			<div className="relative flex items-center justify-end w-full h-full pt-8">
				<motion.div
					animate={{ y: ["4%", "-2%", "4%"] }}
					transition={{ duration: 6, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut", delay: 1 }}
					className="relative w-[140%] h-full min-h-[220px] ml-2 rounded-tl-xl border-t border-l border-border/50 shadow-[0_0_40px_-10px_rgba(0,0,0,0.5)] dark:shadow-[0_0_40px_-10px_rgba(255,255,255,0.05)] bg-card/50 overflow-hidden translate-x-[5%]"
				>
					<img
		...
		<div className="flex flex-col h-full w-full overflow-hidden">
			<div className="relative z-10 pt-8 px-8 flex flex-col items-center">
				<motion.div
					...
					className="flex size-12 items-center justify-center rounded-full border bg-card shadow-xs outline outline-border/80 outline-offset-2 mb-4"
				>
				...
				<div className="space-y-2 text-center">
					...
					<FeatureDescription>
						Deploy instantly and safely via single sign-on.
					</FeatureDescription>
				</div>
			</div>
			<div className="relative flex-1 flex items-center justify-center min-h-[220px]">
				...
				<div className="relative flex items-center justify-center w-[120%] h-[120%] mt-8 -mb-16">
					<motion.div
